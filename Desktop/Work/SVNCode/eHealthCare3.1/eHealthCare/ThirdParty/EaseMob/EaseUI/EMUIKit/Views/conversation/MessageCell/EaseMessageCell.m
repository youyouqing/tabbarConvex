...
+ (void)initialize
{
    ...
    //cell.leftBubbleMargin = UIEdgeInsetsMake(4, 11, 4, 4);
    //cell.rightBubbleMargin = UIEdgeInsetsMake(4, 4, 4, 11);
    //cell.bubbleMargin = UIEdgeInsetsMake(8, 0, 8, 0);
    
    cell.messageTextFont = [UIFont systemFontOfSize:15];
//    cell.messageTextColor = BLACKCOLOR;
        ...
        switch (messageType) {
            case EMMessageBodyTypeText:
            {
                ...
                if (isSender) {
                    ...
                    _messageTextColor = BLACKCOLOR;//**环信-更改左右消息的颜色
                }
            ...
            case EMMessageBodyTypeVoice:
            {
                ...
                if (isSender) {
                    ...
                    _messageVoiceDurationColor = NAVICOLOR;
                }//**环信--语音事件的字体颜色
            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ?[UIImage imageNamed:@"voice3"]: [UIImage imageNamed:@"voice3-gray"];
                    }
                }
...
- (void)setRightBubbleMargin:(UIEdgeInsets)rightBubbleMargin
{
    ...
    //**环信--改变图片距离背景的宽度
    if (_messageType == EMMessageBodyTypeImage) {
        _rightBubbleMargin = UIEdgeInsetsMake(5.5, 2.5, 5, 10.5);
    }else{
...
- (void)setLeftBubbleMargin:(UIEdgeInsets)leftBubbleMargin
{
    ...
    //**环信--改变图片距离背景的宽度
    if (_messageType == EMMessageBodyTypeImage) {
         _leftBubbleMargin = UIEdgeInsetsMake(5.5, 10.5, 5, 2.5);
    }else{
...
- (void)setMessageVoiceDurationColor:(UIColor *)messageVoiceDurationColor
{
    ...
    if (_bubbleView.voiceDurationLabel) {
        _bubbleView.voiceDurationLabel.textColor = messageVoiceDurationColor;
    }
}
