...
#import "XKDVSendCell.h"
#import "XKDVResiveCell.h"
...
- (void)viewDidLoad {
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"chat_send"] stretchableImageWithLeftCapWidth:15 topCapHeight:25]];//**环信发送消息的背景图片
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"chat_send-white"] stretchableImageWithLeftCapWidth:25 topCapHeight:25]];//**环信接收消息的背景图片
    
    [[EaseBaseMessageCell appearance] setSendMessageVoiceAnimationImages:@[[UIImage imageNamed:@"voice3"],[UIImage imageNamed:@"EaseUIResource.bundle/chat_receiver_audio_playing000"], [UIImage imageNamed:@"voice1"], [UIImage imageNamed:@"voice2"], [UIImage imageNamed:@"voice3"]]];
    [[EaseBaseMessageCell appearance] setRecvMessageVoiceAnimationImages:@[[UIImage imageNamed:@"voice3-gray"], [UIImage imageNamed:@"EaseUIResource.bundle/chat_sender_audio_playing_000"], [UIImage imageNamed:@"voice1-gray"], [UIImage imageNamed:@"voice2-gray"], [UIImage imageNamed:@"voice3-gray"]]];
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"XKDVSendCell" bundle:nil] forCellReuseIdentifier:@"dvSend"];
    [self.tableView registerNib:[UINib nibWithNibName:@"XKDVResiveCell" bundle:nil] forCellReuseIdentifier:@"dvResive"];
...
{
    id object = [self.dataArray objectAtIndex:indexPath.row];
//    tableView.backgroundColor = COLOR(230, 235, 235, 1.0);
    //time cell
    if ([object isKindOfClass:[NSString class]]) {
...
//        timeCell.titleLabelColor = [UIColor whiteColor];//时间的字体颜色
        return timeCell;
    }
    else{
        id<IMessageModel> model = object;
         EaseMessageModel *modelO=model;
        if (modelO.message.ext && [modelO.message.ext[@"URL"] boolValue]==true) {
            
            if (modelO.isSender) {//发送者消息框
                
                XKDVResiveCell *resiveCell=[tableView dequeueReusableCellWithIdentifier:@"dvResive"];
                
                resiveCell.sendMsg=modelO;
                 resiveCell.sendViewC = self;
                resiveCell.selectionStyle=UITableViewCellSelectionStyleNone;
                return resiveCell;
                
            }else{//接受者消息框
                
                
                XKDVSendCell *sendCell=[tableView dequeueReusableCellWithIdentifier:@"dvSend"];
                
                sendCell.sendMsg=modelO;
                sendCell.sendViewC = self;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        id<IMessageModel> model = object;
        EaseMessageModel *mod = model;
        
        if (mod.message.ext && [mod.message.ext[@"URL"] boolValue] == true) {
...
//    NSDictionary *dict=[[defaluts objectForKey:@"XKPersonmineInfo"] objectForKey:@"Result"];
    //3.0之前的本本
//    NSString *str=@"0";
//    if ([[NSString stringWithFormat:@"%@",[dict objectForKey:@"Sex"]] isEqualToString:@"男"]) {
//        str=@"0";
//    }else{
//        str=@"1";
//    }
    //3.0之后的版本
    NSString *str=@"0";
    if ([UserInfoTool getLoginInfo].SexID == 0) {//男的
        str=@"0";
    }else{
        str=@"1";
    }
    ...
    //3.0之前的版本
//    NSString *headimag=[dict objectForKey:@"HeadImg"]?[dict objectForKey:@"HeadImg"]:@" ";
    //3.0之后的版本
    NSString *headimag=[UserInfoTool getLoginInfo].HeadImg?[UserInfoTool getLoginInfo].HeadImg:@" ";
    //3.0之前的版本
//    NSString *name=[dict objectForKey:@"Name"]?[NSString stringWithFormat:@"%@",[dict objectForKey:@"Name"]]:@" ";
    //3.0之后的版本
    NSString *name=[UserInfoTool getLoginInfo].Name?[UserInfoTool getLoginInfo].Name:@" ";
    
    NSString *phone=[UserInfoTool getLoginInfo].Mobile?[UserInfoTool getLoginInfo].Mobile:@" ";
    
    //3.0之前的版本
//    NSString *age=[NSString stringWithFormat:@"%@",dict[@"Age"]];
    //3.0之后的版本
    NSString *age=[NSString stringWithFormat:@"%li",[UserInfoTool getLoginInfo].Age];
    
    NSString *RecordID=[NSString stringWithFormat:@"%@",[UserInfoTool getLoginInfo].RecordID];
    NSDictionary *dict1=@{@"headimager":headimag,@"name":name,@"phone":phone,@"sex":str,@"age":age,@"MemberID":[NSString stringWithFormat:@"%li",[UserInfoTool getLoginInfo].MemberID],@"RecordID":RecordID};
    XKLOG(@"%@",dict1);
    NSDictionary *ext = dict1;
    if (self.conversation.type == EMConversationTypeGroupChat) {
