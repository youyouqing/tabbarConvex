...
- (void)LQPhotoPicker_initPickerView{
    ...
    pushImgName = @"Pop-up-window-add.png";
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == _LQPhotoPicker_smallImageArray.count) {
        ...
        if (indexPath.row == self.LQPhotoPicker_imgMaxCount) {
...
{
    return CGSizeMake((self.LQPhotoPicker_superView.frame.size.width-50) /4 ,(self.LQPhotoPicker_superView.frame.size.width-50) /4);//**cell
}
...
- (void)changeCollectionViewHeight{
    ...
    if (_collectionFrameY) {
        _pickerCollectionView.frame = CGRectMake(0, _collectionFrameY, self.LQPhotoPicker_superView.frame.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /4.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/4 +1)+0);
    }
    else{
        _pickerCollectionView.frame = CGRectMake(0, 0, self.LQPhotoPicker_superView.frame.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /4.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/4 +1)+0);
    }
...
- (void)LQPhotoPicker_updatePickerViewFrameY:(CGFloat)Y{
    ...
    _collectionFrameY = Y;
    _pickerCollectionView.frame = CGRectMake(0, Y, self.LQPhotoPicker_superView.frame.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /4.0 +20.0)* ((int)(_LQPhotoPicker_selectedAssetArray.count)/4 +1)+0);
}
