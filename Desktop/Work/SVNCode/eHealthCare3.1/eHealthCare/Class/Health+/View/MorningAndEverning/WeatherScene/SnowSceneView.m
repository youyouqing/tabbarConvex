...
//    NSString * const kCAEmitterLayerAdditive; //叠加效果
    ...
    //发射源
    //背景雪
    CAEmitterCell* backgroundEmitterCell = [self snowBackgroundEmitterCellimgName:imgName SceneType:type];
    //前景雪
    CAEmitterCell* forgroundEmitterCell = [self snowForgroundEmitterCellimgName:imgName SceneType:type];
    
    //add the cell to the emitter layer
    self.emitter.emitterCells = @[backgroundEmitterCell, forgroundEmitterCell];
    
//    if (type == HeavyRainScene) {
//      self.emitter.emitterCells = @[ forgroundEmitterCell];
//    }
}


//雪的最前面一层，少数较大雪花效果
- (CAEmitterCell *)snowForgroundEmitterCellimgName:(NSString *)imgName SceneType:(WeatherSceneType)type{
    //create new emitter cell
    CAEmitterCell* emitterCell = [CAEmitterCell emitterCell];
    //粒子图片
    emitterCell.contents = (id)[[UIImage imageNamed:imgName] CGImage];
    //粒子源名称
    ...
    if (type == HeavyRainScene) {
        ...
        //速度范围
        emitterCell.velocityRange = 1000;
    }
    else
    {
        //产生速率
...
//雪的后面一层，较小雪花效果
- (CAEmitterCell *)snowBackgroundEmitterCellimgName:(NSString *)imgName SceneType:(WeatherSceneType)type{
    //create new emitter cell
    CAEmitterCell* emitterCell = [CAEmitterCell emitterCell];
