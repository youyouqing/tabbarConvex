...
@property (nonatomic,assign) CoreNetWorkStatus currentNetWorkStatus;
...
+ (void)initialize{
    ...
    status.currentNetWorkStatus = [[CoreStatus sharedCoreStatus] statusWithRadioAccessTechnology];
...
+ (CoreNetWorkStatus)currentNetWorkStatus{
    ...
    CoreStatus *status = [CoreStatus sharedCoreStatus];
    return status.currentNetWorkStatus;
}
...
{
    if(self.currentNetWorkStatus == [[CoreStatus sharedCoreStatus] statusWithRadioAccessTechnology]) return;
    
    //发送通知
    self.currentNetWorkStatus = [[CoreStatus sharedCoreStatus] statusWithRadioAccessTechnology];
    ...
    if (_netWorkNotiBlock) {
        _netWorkNotiBlock(self.currentNetWorkStatus);
    }
}
