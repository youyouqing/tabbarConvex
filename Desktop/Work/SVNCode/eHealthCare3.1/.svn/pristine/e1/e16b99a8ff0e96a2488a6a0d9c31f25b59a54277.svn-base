...
+ (void)initialize
{
    ...
    cell.messageNameColor = [UIColor clearColor];
    cell.messageNameFont = [UIFont systemFontOfSize:10];
    cell.messageNameHeight = 10;//姓名的高度
...
//        _nameLabel.textColor = [UIColor clearColor];//隐藏名字
...
- (void)setModel:(id<IMessageModel>)model
{
    ...
    //**环信--聊天界面头像
    if (model.isSender) {
        //3.0之前的版本取 dic[@"Headimg"]
//        NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
//        NSDictionary *dic = [user objectForKey:@"XKPersonmineInfo"];

        [self.avatarView  sd_setImageWithURL:[NSURL URLWithString:[UserInfoTool getLoginInfo].HeadImg] placeholderImage:[UIImage imageNamed:@"defaultHead"]];
        
    }else{
        [[UserEaseInfoManager shareInstance] getNicknameByUserNameDic:model.nickname cellInfoDic:^(id json){
            NSDictionary *doctorDic = (NSDictionary *)json;
            UserEaseInfoModel *doctorModel = [UserEaseInfoModel objectWithKeyValues:doctorDic];
            [self.avatarView sd_setImageWithURL:[NSURL URLWithString:doctorModel.DoctorImg] placeholderImage:[UIImage imageNamed:@"defaultHead"]];
