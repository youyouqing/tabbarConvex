...
- (void)initSubViews {
    ...
    [self.movLine setBackgroundColor:kMainColor];
}
...
- (void)updateUnderLinePos {
    if( self.curButton ) {
        ...
        [UIView animateWithDuration:0.1 animations:^{
            [self.movLine setFrame:CGRectMake(CGRectGetMinX(self.movLine.frame), KHeight(45)-2, lWidth, CGRectGetHeight(self.movLine.frame))];
...
- (UIView *)movLine {
    if( !_movLine ) {
        _movLine = [[UIView alloc]initWithFrame:CGRectMake(0, 0, KHeight(45)-2, 2)];
    }
    return _movLine;
}
@end
