...
@interface HealthPlanSubViewController () <UITableViewDelegate,UITableViewDataSource>
...
@implementation HealthPlanSubViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self getHealthPlanListWithNetWorking];
}
...
//获取列表数据
- (void)getHealthPlanListWithNetWorking
{
    NSDictionary *dic = @{@"Token":[UserInfoTool getLoginInfo].Token,
                          @"PlanTypeID":self.PlanTypeID,
                          @"MemberID":@([UserInfoTool getLoginInfo].MemberID),
                          @"PageIndex":@(self.pageIndex),
                          @"PageSize":@(10)};
    
    [HealthPlanViewModel getHealthPlanCategoryListWithParams:dic FinishedBlock:^(ResponseObject *response) {
