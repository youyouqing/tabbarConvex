...
- (UIColor *)selColor
{
    if (_selColor == nil) self.selColor = [UIColor yellowColor];
...
- (UIView *)underLine
{
    if (_underLine == nil) {
        ...
        underLineView.backgroundColor = _underLineColor?_underLineColor:kMainColor;
...
- (UIView *)line
{
    if (_line == nil) {
        ...
        _line.backgroundColor = [UIColor getColor:@"d8d8d8"];
...
//      _contentScrollView.scrollEnabled = NO;// 禁止底部滑动
        // 注册cell
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        if (_isfullScreen) {
            ...
            self.line.frame = CGRectMake(0, titleY+self.titleHeight-0.5, self.titleScrollView.frame.size.width*5, 0.5);
...
- (void)setUpAllTitle
{
    ...
    _contentScrollView.contentSize = CGSizeMake(count * YZScreenW, 0);
      ...
      self.line.width = self.titleLabels.count*_titleWidth;
