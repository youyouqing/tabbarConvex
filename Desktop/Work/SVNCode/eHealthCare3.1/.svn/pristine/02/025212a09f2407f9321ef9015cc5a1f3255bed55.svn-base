...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    CGFloat insets = (self.frame.size.width - 4 * CHAT_BUTTON_SIZE) / 5;//button之间的间距
...
//    [_scrollview addSubview:_locationButton];//隐藏发送位置按钮，下面的button的frame也随之改变，若需还原frame可对比官方demo进行对比
    ...
    if (type == EMChatToolbarTypeChat) {
        frame.size.height = 150/2;//修改scrollview的高度 /2是
        _audioCallButton =[UIButton buttonWithType:UIButtonTypeCustom];
        [_audioCallButton setFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE * 2, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
...
//        [_scrollview addSubview:_audioCallButton];
        ...
        [_videoCallButton setFrame:CGRectMake(insets * 4 + CHAT_BUTTON_SIZE * 3, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
